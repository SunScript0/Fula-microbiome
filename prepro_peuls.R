...
setwd("C:/Users/mora_/Desktop/Peuls/My analysis 2")
...
# Base metadata
meta = read.table(paste0(datapath, "metadata.tsv"), 
                  row.names=1, fill=T, header = T, sep="\t")
# Read counts
meta_dada2 = read.table(paste0(datapath,"dada2_summary.tsv"), 
                        row.names=1, fill=T, header = T, sep="\t")
...
save(ab, taxa, tree, meta, file = paste0(datapath, "fula_data.Rdata"))
